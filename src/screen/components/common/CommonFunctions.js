...
async function tokenRefresh() {
  if (!window.parent.fetchTriggered) {
      window.parent.fetchTriggered = true;
      ...
      try {
          ...
          window.parent.application.context.renewToken(data);
          window.parent.fetchTriggered = false;
      } catch (error) {
  ...
  try {
      ...
      if (response.status === 401) {
          ...
          const renewedOptions = {
              ...
              headers: {
                  ...options.headers,
                  Authorization: 'Bearer ' + JSON.parse(localStorage.jStorage)[window.parent.application.context.get_apiTokenKey()],
              },
          };
